...
class VideoCutBar @JvmOverloads constructor(
    ...
    private fun invalidateCenterThumbWithProgress() {
        ...
        controllerListener?.onProcessToPixel(rectThumbProgress.centerX() - rectThumbRight.width() * 1.3f)
    }
        ...
        event?.let {
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    controllerListener?.onTouchDown(convertPositionToProgress(event.x), rectCutBar.contains(event.x, event.y))
                    true
                }
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    controllerListener?.onTouchTouchRelease(convertPositionToProgress(event.x), rectCutBar.contains(event.x, event.y))
                    false
                }
                else -> true
            }
    ...
    fun setRangeChanged(min: Long, max: Long) {
        rangeChangeListener?.onRangeChanged(this, min, max, thumbIndex)
        invalidate()
    }

    private fun convertPositionToProgress(pixel: Float): Float {
        return pixel / rectCutBar.width() * duration
    }
    ...
    fun cancelLoading() = com.solar.dev.tiktok.app.ui.customview.cancelLoading()
    ...
    interface ControllerListener {
        fun onProcessToPixel(pixel: Float)

        fun onTouchDown(touchProgress: Float, touchInBar: Boolean)

        fun onTouchTouchRelease(touchProgress: Float, touchInBar: Boolean)
    }
