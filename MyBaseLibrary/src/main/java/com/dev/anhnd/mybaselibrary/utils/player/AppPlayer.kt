package com.dev.anhnd.mybaselibrary.utils.player
...
class AppPlayer : LifecycleObserver {
    ...
    fun init(videoIdRes: Int) {
        liveState.value = State.NOT_READY
        isIniting = true
        listener?.onLoadStart()
        currentProgress = 0
        media = SimpleExoPlayer.Builder(getApplication(), DefaultRenderersFactory(getApplication())).build()
        val dataSourceFactory = DefaultDataSourceFactory(
            getApplication(),
            Util.getUserAgent(
                getApplication(),
        ...
        val uri = RawResourceDataSource.buildRawResourceUri(videoIdRes)
        val mediaSource = ProgressiveMediaSource.Factory(dataSourceFactory).createMediaSource(uri)
        initWithMediaSource(mediaSource)
    }
    ...
    private fun initWithMediaSource(mediaSource: ProgressiveMediaSource) {
        media?.volume = 0f
        media?.addListener(object : Player.EventListener {
